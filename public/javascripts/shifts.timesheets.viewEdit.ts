...
import type { DoGetShiftTimesheetsResponse } from '../../handlers/shifts-post/doGetShiftTimesheets.js'
...
;(() => {
  ...
  const shiftId = (
    document.querySelector('#shift--shiftId') as HTMLInputElement
  ).value

  const urlPrefix = `${shiftLog.urlPrefix}/${shiftLog.shiftsRouter}`

  function loadShiftTimesheets(): void {
