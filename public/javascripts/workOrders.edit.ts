...
;(() => {
  ...
  const originalWorkOrderCloseDateTime =
  ...
  function submitWorkOrderUpdate(): void {
    cityssm.postJSON(
      `${workOrderUrlPrefix}/${isCreate ? 'doCreateWorkOrder' : 'doUpdateWorkOrder'}`,
      workOrderFormElement,
      (rawResponseJSON: DoCreateWorkOrderResponse | DoUpdateWorkOrderResponse) => {
   ...
   */
  const requestorNameInput = workOrderFormElement.querySelector(
    '#workOrder--requestorName'
  ) as HTMLInputElement

  const requestorContactInfoInput = workOrderFormElement.querySelector(
    '#workOrder--requestorContactInfo'
  ) as HTMLInputElement
  ...
  if (requestorDatalist !== null) {
    ...
    requestorNameInput.addEventListener('keyup', () => {
      ...
      if (
        ...
        cityssm.postJSON(
          ...
          (rawResponseJSON: DoGetRequestorSuggestionsResponse) => {
    ...
    requestorNameInput.addEventListener('change', () => {
      ...
      const matchingRequestor = requestorsData.find(
        (possibleRequestor) => possibleRequestor.requestorName === selectedName
      )
  ...
  if (locationDatalist !== null) {
    ...
    function fetchLocationSuggestions(
      ...
      cityssm.postJSON(
        ...
        { searchString },
        (rawResponseJSON: DoGetLocationSuggestionsResponse) => {
    ...
    locationAddress1Input.addEventListener('keyup', () => {
      ...
      if (
        newSearchString.length >= LOCATION_SEARCH_MIN_LENGTH &&
      ...
      if (matchingLocation !== undefined) {
        ...
        if (hasExistingData) {
          bulmaJS.confirm({
            ...
            message:
              'Do you want to update the location details with the selected address?',
            okButton: {
              text: 'Update',
    ...
    function applyLocationData(location: Location): void {
        ...
        okButton: {
          ...
          callbackFunction: () => {
            cityssm.postJSON(
              ...
              (rawResponseJSON: DoDeleteWorkOrderResponse) => {
