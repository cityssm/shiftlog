...
;(() => {
  ...
  const userGroupsContainerElement = document.querySelector(
    '#container--userGroups'
  ) as HTMLElement

  function deleteUserGroup(clickEvent: Event): void {
  ...
  function editUserGroup(clickEvent: Event): void {
    const buttonElement = clickEvent.currentTarget as HTMLButtonElement
    const userGroupId = buttonElement.dataset.userGroupId
    ...
    function doUpdateUserGroup(submitEvent: Event): void {
  ...
  function manageMembers(clickEvent: Event): void {
    const buttonElement = clickEvent.currentTarget as HTMLButtonElement
    const userGroupId = buttonElement.dataset.userGroupId
    ...
    */
    ...
    function addMember(submitEvent: Event): void {
      submitEvent.preventDefault()

      const addForm = submitEvent.currentTarget as HTMLFormElement
      const userName = (
        addForm.querySelector('[name="userName"]') as HTMLSelectElement
      ).value
    ...
    function removeMember(removeMemberClickEvent: Event): void {
      ...
      cityssm.postJSON(
        `${shiftLog.urlPrefix}/admin/doDeleteUserGroupMember`,
        {
          userGroupId,
          userName
        },
        (rawResponseJSON) => {
          const responseJSON =
            rawResponseJSON as DoDeleteUserGroupMemberResponse
    ...
    cityssm.openHtmlModal('adminUserGroups-members', {
      onshow(modalElement) {
        ;(
          modalElement.querySelector('#span--groupName') as HTMLSpanElement
        ).textContent = userGroup.userGroupName
  ...
  function buildUserGroupRowElement(userGroup: UserGroup): HTMLTableRowElement {
  ...
  document
    ...
    ?.addEventListener('click', () => {
      ...
      function doAddUserGroup(submitEvent: Event): void {
        submitEvent.preventDefault()
