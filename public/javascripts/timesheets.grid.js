...
var TimesheetGrid = /** @class */ (function () {
    ...
    TimesheetGrid.prototype.addRow = function () {
        var _this = this;
        var closeModalFunction;
        var doAddRow = function (submitEvent) {
            submitEvent.preventDefault();
            var addForm = submitEvent.currentTarget;
            cityssm.postJSON("".concat(_this.shiftLog.urlPrefix, "/").concat(_this.shiftLog.timesheetsRouter, "/doAddTimesheetRow"), addForm, function (rawResponseJSON) {
                var result = rawResponseJSON;
                if (result.success) {
                    closeModalFunction();
                    _this.loadData().then(function () {
        ...
        cityssm.postJSON("".concat(this.shiftLog.urlPrefix, "/").concat(this.shiftLog.timesheetsRouter, "/doGetTimesheetRowOptions"), {}, function (rawResponseJSON) {
            var optionsData = rawResponseJSON;
            ...
            cityssm.openHtmlModal('timesheets-addRow', {
                onshow: function (modalElement) {
                    ...
                    // Set the timesheet ID
                    ;
                    modalElement.querySelector('#addTimesheetRow--timesheetId').value = _this.config.timesheetId.toString();
                    modalElement.querySelector('#addTimesheetRow--rowTitle').value = '';
                    // Populate employees
                    var employeeSelect = modalElement.querySelector('#addTimesheetRow--employeeNumber');
                    employeeSelect.innerHTML = '<option value="">(None)</option>';
                    for (var _i = 0, _b = optionsData.employees; _i < _b.length; _i++) {
                        var employee = _b[_i];
                        employeeSelect.insertAdjacentHTML('beforeend', "<option value=\"".concat(cityssm.escapeHTML(employee.employeeNumber), "\">").concat(cityssm.escapeHTML(employee.lastName), ", ").concat(cityssm.escapeHTML(employee.firstName), " (").concat(cityssm.escapeHTML(employee.employeeNumber), ")</option>"));
                    }
                    // Populate equipment
                    var equipmentSelect = modalElement.querySelector('#addTimesheetRow--equipmentNumber');
                    equipmentSelect.innerHTML = '<option value="">(None)</option>';
                    for (var _c = 0, _d = optionsData.equipment; _c < _d.length; _c++) {
                        var equip = _d[_c];
                        equipmentSelect.insertAdjacentHTML('beforeend', "<option value=\"".concat(cityssm.escapeHTML(equip.equipmentNumber), "\">").concat(cityssm.escapeHTML(equip.equipmentName), " (").concat(cityssm.escapeHTML(equip.equipmentNumber), ")</option>"));
                    }
                    // Populate job classifications
                    var jobClassSelect = modalElement.querySelector('#addTimesheetRow--jobClassificationDataListItemId');
                    jobClassSelect.innerHTML = '<option value="">(None)</option>';
                    for (var _e = 0, _f = optionsData.jobClassifications; _e < _f.length; _e++) {
                        var jobClass = _f[_e];
                        jobClassSelect.insertAdjacentHTML('beforeend', "<option value=\"".concat(jobClass.dataListItemId.toString(), "\">").concat(cityssm.escapeHTML(jobClass.dataListItem), "</option>"));
                    }
                    // Populate time codes
                    var timeCodeSelect = modalElement.querySelector('#addTimesheetRow--timeCodeDataListItemId');
                    timeCodeSelect.innerHTML = '<option value="">(None)</option>';
                    for (var _g = 0, _h = optionsData.timeCodes; _g < _h.length; _g++) {
                        var timeCode = _h[_g];
                        timeCodeSelect.insertAdjacentHTML('beforeend', "<option value=\"".concat(timeCode.dataListItemId.toString(), "\">").concat(cityssm.escapeHTML(timeCode.dataListItem), "</option>"));
                    }
                    // Attach form submit handler
                    (_a = modalElement.querySelector('form')) === null || _a === void 0 ? void 0 : _a.addEventListener('submit', doAddRow);
                },
                onshown: function (_modalElement, closeFunction) {
                    bulmaJS.toggleHtmlClipped();
                    closeModalFunction = closeFunction;
    ...
    TimesheetGrid.prototype.editRow = function (row) {
        var _this = this;
        var closeModalFunction;
        var doUpdateRow = function (submitEvent) {
            submitEvent.preventDefault();
            var editForm = submitEvent.currentTarget;
            cityssm.postJSON("".concat(_this.shiftLog.urlPrefix, "/").concat(_this.shiftLog.timesheetsRouter, "/doUpdateTimesheetRow"), editForm, function (rawResponseJSON) {
                var result = rawResponseJSON;
                if (result.success) {
                    closeModalFunction();
                    _this.loadData().then(function () {
        ...
        cityssm.postJSON("".concat(this.shiftLog.urlPrefix, "/").concat(this.shiftLog.timesheetsRouter, "/doGetTimesheetRowOptions"), {}, function (rawResponseJSON) {
            ...
            cityssm.openHtmlModal('timesheets-editRow', {
                onshow: function (modalElement) {
                    ...
                    var employeeDisplay = modalElement.querySelector('#editTimesheetRow--employeeDisplay');
                    if (row.employeeNumber !== undefined && row.employeeNumber !== null) {
                        employeeDisplay.value = "".concat((_a = row.employeeLastName) !== null && _a !== void 0 ? _a : '', ", ").concat((_b = row.employeeFirstName) !== null && _b !== void 0 ? _b : '', " (").concat(row.employeeNumber, ")");
                    }
                    else {
                        employeeDisplay.value = '(None)';
                    }
                    // Display equipment (read-only)
                    var equipmentDisplay = modalElement.querySelector('#editTimesheetRow--equipmentDisplay');
                    if (row.equipmentNumber !== undefined && row.equipmentNumber !== null) {
                        equipmentDisplay.value = "".concat((_c = row.equipmentName) !== null && _c !== void 0 ? _c : '', " (").concat(row.equipmentNumber, ")");
                    }
                    else {
                        equipmentDisplay.value = '(None)';
                    }
                    // Populate job classifications
                    var jobClassSelect = modalElement.querySelector('#editTimesheetRow--jobClassificationDataListItemId');
                    jobClassSelect.innerHTML = '<option value="">(None)</option>';
                    for (var _i = 0, _e = optionsData.jobClassifications; _i < _e.length; _i++) {
                        var jobClass = _e[_i];
                        var selected = row.jobClassificationDataListItemId === jobClass.dataListItemId;
                        jobClassSelect.insertAdjacentHTML('beforeend', "<option value=\"".concat(jobClass.dataListItemId.toString(), "\"").concat(selected ? ' selected' : '', ">").concat(cityssm.escapeHTML(jobClass.dataListItem), "</option>"));
                    }
                    // Populate time codes
                    var timeCodeSelect = modalElement.querySelector('#editTimesheetRow--timeCodeDataListItemId');
                    timeCodeSelect.innerHTML = '<option value="">(None)</option>';
                    for (var _f = 0, _g = optionsData.timeCodes; _f < _g.length; _f++) {
                        var timeCode = _g[_f];
                        var selected = row.timeCodeDataListItemId === timeCode.dataListItemId;
                        timeCodeSelect.insertAdjacentHTML('beforeend', "<option value=\"".concat(timeCode.dataListItemId.toString(), "\"").concat(selected ? ' selected' : '', ">").concat(cityssm.escapeHTML(timeCode.dataListItem), "</option>"));
                    }
                    // Attach form submit handler
                    (_d = modalElement.querySelector('form')) === null || _d === void 0 ? void 0 : _d.addEventListener('submit', doUpdateRow);
                },
                onshown: function (_modalElement, closeFunction) {
                    bulmaJS.toggleHtmlClipped();
                    closeModalFunction = closeFunction;
    ...
    TimesheetGrid.prototype.deleteRow = function (row) {
        var _this = this;
        var rowTotal = this.getRowTotal(row.timesheetRowId);
        var message = 'Are you sure you want to delete this row?';
        if (rowTotal > 0) {
            message = "<strong>Warning:</strong> This row has <strong>".concat(rowTotal, " recorded hours</strong>. All associated hours will be permanently lost.<br><br>Are you sure you want to delete this row?");
        }
        bulmaJS.confirm({
            title: 'Delete Row',
            message: message,
