...
import type { DoGetWorkOrdersForPlannerResponse } from '../../handlers/workOrders-post/doGetWorkOrdersForPlanner.js'
...
;(() => {
  ...
  const filtersFormElement = document.querySelector(
    '#form--workOrderPlanner'
  ) as HTMLFormElement

  const dateFilterElement = document.querySelector(
    '#workOrderPlanner--dateFilter'
  ) as HTMLSelectElement

  const daysThresholdElement = document.querySelector(
  ...
  const resultsContainerElement = document.querySelector(
    '#container--workOrderPlannerResults'
  ) as HTMLDivElement
  ...
  function renderWorkOrdersTable(
    data: DoGetWorkOrdersForPlannerResponse
  ): void {
    ...
    for (const workOrder of data.workOrders) {
      ...
      const isOverdue =
        ...
        new Date(workOrder.workOrderDueDateTime as string) < now
      ...
      const openDate = cityssm.dateToString(
        new Date(workOrder.workOrderOpenDateTime as string)
      )
      const daysOpen = Math.floor(
        (now.getTime() -
          new Date(workOrder.workOrderOpenDateTime as string).getTime()) /
          (1000 * 60 * 60 * 24)
      )
  ...
  function getPlannerResults(): void {
    resultsContainerElement.innerHTML = /* html */ `
    ...
    cityssm.postJSON(
      ...
      requestData,
      (rawResponseJSON: DoGetWorkOrdersForPlannerResponse) => {
