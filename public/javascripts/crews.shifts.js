(() => {
    ...
    function renderCrewDetails(crewId, crew) {
        ...
        // Render equipment
        let equipmentHTML = '<div class="panel-block"><strong>Equipment</strong></div>';
        if (crew.equipment.length === 0) {
            ...
            for (const equipmentItem of crew.equipment) {
                ...
                if (canEdit) {
                    ...
                    for (const member of crew.members) {
                        equipmentHTML += `<option value="${cityssm.escapeHTML(member.employeeNumber)}"${equipmentItem.employeeNumber === member.employeeNumber
                            ? ' selected'
        ...
        // Add event listeners
        if (canEdit) {
            for (const buttonElement of detailsElement.querySelectorAll('[data-delete-member]')) {
                buttonElement.addEventListener('click', deleteCrewMember);
            }
            for (const buttonElement of detailsElement.querySelectorAll('[data-add-member]')) {
                buttonElement.addEventListener('click', openAddCrewMemberModal);
            }
            for (const buttonElement of detailsElement.querySelectorAll('[data-delete-equipment]')) {
                buttonElement.addEventListener('click', deleteCrewEquipment);
            }
            for (const buttonElement of detailsElement.querySelectorAll('[data-add-equipment]')) {
                buttonElement.addEventListener('click', openAddCrewEquipmentModal);
            }
            for (const selectElement of detailsElement.querySelectorAll('[data-update-assignment]')) {
    ...
    function renderCrews() {
        ...
        // Add event listeners
        for (const linkElement of crewsContainerElement.querySelectorAll('[data-expand-crew]')) {
            linkElement.addEventListener('click', (clickEvent) => {
                clickEvent.preventDefault();
                expandCrewPanel(clickEvent);
            });
        }
        for (const buttonElement of crewsContainerElement.querySelectorAll('[data-edit-crew]')) {
            buttonElement.addEventListener('click', openEditCrewModal);
        }
        for (const buttonElement of crewsContainerElement.querySelectorAll('[data-delete-crew]')) {
