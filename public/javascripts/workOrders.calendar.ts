...
;(() => {
   ...
   */
  function getEventTypeLeftIcon(eventType: string): string {
  ...
  function getEventStatus(
    ...
    function getOpenOrOverdueStatus(
      dueDateTime: Date | string | null | undefined,
      currentDate: Date
    ): { statusText: string; rightTagClass: string } {
      let statusText = 'Open'
      let rightTagClass: string
  ...
  function renderCalendar(events: WorkOrderCalendarEvent[]): void {
    ...
    for (const dayName of dayNames) {
      calendarElement
        ...
        ?.insertAdjacentHTML(
          'beforeend',
          `<th class="has-text-centered">${cityssm.escapeHTML(dayName)}</th>`
        )
    }
  ...
  function loadCalendar(): void {
    ...
    cityssm.postJSON(
      ...
      (rawResponseJSON: DoGetCalendarEventsResponse) => {
        const responseJSON = rawResponseJSON as DoGetCalendarEventsResponse
