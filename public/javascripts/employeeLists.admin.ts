...
;(() => {
  ...
  const employeeListsContainerElement = document.querySelector(
    '#container--employeeLists'
  ) as HTMLElement

  // Track Sortable instances to prevent duplicates
  const sortableInstances = new Map<string, SortableInstance>()

  function deleteEmployeeList(clickEvent: Event): void {
    const buttonElement = clickEvent.currentTarget as HTMLButtonElement

    const employeeListId = Number.parseInt(
      buttonElement.dataset.employeeListId ?? '',
      10
    )

    const employeeList = exports.employeeLists.find(
      (list) => list.employeeListId === employeeListId
    )
    ...
    bulmaJS.confirm({
      ...
      okButton: {
        ...
        callbackFunction() {
          cityssm.postJSON(
            `${shiftLog.urlPrefix}/admin/doDeleteEmployeeList`,
            {
              employeeListId
            },
            (rawResponseJSON) => {
              const responseJSON =
                rawResponseJSON as DoDeleteEmployeeListResponse
  ...
  function editEmployeeList(clickEvent: Event): void {
    const buttonElement = clickEvent.currentTarget as HTMLButtonElement
    const employeeListId = Number.parseInt(
      buttonElement.dataset.employeeListId ?? '',
      10
    ...
    function doEdit(submitEvent: Event): void {
    ...
    cityssm.openHtmlModal('adminEmployeeLists-edit', {
      onshow(modalElement) {
        ...
        ;(
          formElement.querySelector(
            '#employeeListEdit--employeeListName'
          ) as HTMLInputElement
        ).value = employeeList.employeeListName
        ;(
          formElement.querySelector(
            '#employeeListEdit--userGroupId'
          ) as HTMLSelectElement
        ).value = (employeeList.userGroupId ?? '').toString()
  ...
  function loadEmployeeListDetails(
    employeeListId: number,
    panelElement: HTMLElement
  ): void {
    cityssm.postJSON(
      `${shiftLog.urlPrefix}/admin/doGetEmployeeList`,
      {
        employeeListId
      },
      (rawResponseJSON) => {
  ...
  function deleteEmployeeListMember(
    employeeListId: number,
    employeeNumber: string,
    panelElement: HTMLElement
  ): void {
    cityssm.postJSON(
      `${shiftLog.urlPrefix}/admin/doDeleteEmployeeListMember`,
      {
        employeeListId,
        employeeNumber
      },
      (rawResponseJSON) => {
        const responseJSON =
          rawResponseJSON as DoDeleteEmployeeListMemberResponse
  ...
  function addEmployeeListMember(
    employeeListId: number,
    panelElement: HTMLElement
  ): void {
    let formElement: HTMLFormElement
    ...
    function doAdd(submitEvent: Event): void {
      ...
      cityssm.postJSON(
        ...
        (rawResponseJSON) => {
          const responseJSON =
            rawResponseJSON as DoAddEmployeeListMemberResponse
          ...
          if (responseJSON.success && responseJSON.employeeList !== undefined) {
            ...
            bulmaJS.alert({
              ...
              message:
                'Please try again. The employee may already be in the list.'
            })
          }
        }
      )
    ...
    cityssm.openHtmlModal('adminEmployeeLists-addMember', {
      onshow(modalElement) {
        ...
        const selectElement = formElement.querySelector(
          '#employeeListMember--employeeNumber'
        ) as HTMLSelectElement
  ...
  function editEmployeeListMember(
    ...
    function doUpdate(submitEvent: Event): void {
      ...
      cityssm.postJSON(
        ...
        (rawResponseJSON) => {
          const responseJSON =
            rawResponseJSON as DoUpdateEmployeeListMemberResponse
  ...
  function initializeSortable(
    ...
    const hasItems =
      tbodyElement.querySelectorAll('tr[data-employee-number]').length > 0
    ...
    const sortableInstance = Sortable.create(tbodyElement, {
      ...
      onEnd() {
        ...
        // Send to server
        cityssm.postJSON(
          `${shiftLog.urlPrefix}/admin/doReorderEmployeeListMembers`,
          {
            employeeListId,
            employeeNumbers,
          ...
          (rawResponseJSON) => {
            const responseJSON =
              rawResponseJSON as DoReorderEmployeeListMembersResponse
  ...
  function renderEmployeeListMembers(
    employeeList: EmployeeListWithMembers,
    panelElement: HTMLElement
  ): void {
    const membersContainerElement = panelElement.querySelector(
      '.panel-block-members'
    ) as HTMLElement
    ...
    const membersByDate = new Map<string, EmployeeListMember[]>()
