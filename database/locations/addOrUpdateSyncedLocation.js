...
export default async function addOrUpdateSyncedLocation(partialLocation, syncUserName) {
    const currentLocation = await getLocationByAddress1(partialLocation.address1 ?? '');
    try {
        ...
        debug(`Error adding or updating synced location ${partialLocation.address1}: `, error);
        return false;
    }
    return true;
}
