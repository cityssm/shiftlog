"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.default = reorderEmployeeListMembers;
var mssql_multi_pool_1 = require("@cityssm/mssql-multi-pool");
var config_helpers_js_1 = require("../../helpers/config.helpers.js");
function reorderEmployeeListMembers(form) {
    return __awaiter(this, void 0, void 0, function () {
        var pool, transaction, _i, _a, _b, index, employeeNumber, _c, _d;
        var _e;
        return __generator(this, function (_f) {
            switch (_f.label) {
                case 0:
                    _f.trys.push([0, 12, , 13]);
                    return [4 /*yield*/, mssql_multi_pool_1.default.connect((0, config_helpers_js_1.getConfigProperty)('connectors.shiftLog'))];
                case 1:
                    pool = _f.sent();
                    transaction = pool.transaction();
                    return [4 /*yield*/, transaction.begin()];
                case 2:
                    _f.sent();
                    _f.label = 3;
                case 3:
                    _f.trys.push([3, 9, , 11]);
                    _i = 0, _a = form.employeeNumbers.entries();
                    _f.label = 4;
                case 4:
                    if (!(_i < _a.length)) return [3 /*break*/, 7];
                    _b = _a[_i], index = _b[0], employeeNumber = _b[1];
                    return [4 /*yield*/, transaction
                            .request()
                            .input('instance', (0, config_helpers_js_1.getConfigProperty)('application.instance'))
                            .input('employeeListId', form.employeeListId)
                            .input('employeeNumber', employeeNumber)
                            .input('seniorityDate', (_e = form.seniorityDate) !== null && _e !== void 0 ? _e : null)
                            .input('seniorityOrderNumber', index).query(/* sql */ "\n            update ShiftLog.EmployeeListMembers\n            set seniorityOrderNumber = @seniorityOrderNumber\n            where instance = @instance\n              and employeeListId = @employeeListId\n              and employeeNumber = @employeeNumber\n              ".concat(form.seniorityDate === undefined ? '' : 'and seniorityDate = @seniorityDate', "\n          "))];
                case 5:
                    _f.sent();
                    _f.label = 6;
                case 6:
                    _i++;
                    return [3 /*break*/, 4];
                case 7: return [4 /*yield*/, transaction.commit()];
                case 8:
                    _f.sent();
                    return [2 /*return*/, true];
                case 9:
                    _c = _f.sent();
                    return [4 /*yield*/, transaction.rollback()];
                case 10:
                    _f.sent();
                    return [2 /*return*/, false];
                case 11: return [3 /*break*/, 13];
                case 12:
                    _d = _f.sent();
                    return [2 /*return*/, false];
                case 13: return [2 /*return*/];
            }
        });
    });
}
