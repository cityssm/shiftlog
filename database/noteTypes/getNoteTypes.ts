...
export default async function getNoteTypes(): Promise<NoteTypeWithFields[]> {
  ...
  const noteTypes = noteTypesResult.recordset as NoteType[]
  ...
  const noteTypesWithFields: NoteTypeWithFields[] = noteTypes.map(
    (noteType) => ({
      ...noteType,
