"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.default = addUser;
var mssql_multi_pool_1 = require("@cityssm/mssql-multi-pool");
var config_helpers_js_1 = require("../../helpers/config.helpers.js");
function insertNewUser(newUserName, user) {
    return __awaiter(this, void 0, void 0, function () {
        var currentDate, pool, _a;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    currentDate = new Date();
                    _b.label = 1;
                case 1:
                    _b.trys.push([1, 4, , 5]);
                    return [4 /*yield*/, mssql_multi_pool_1.default.connect((0, config_helpers_js_1.getConfigProperty)('connectors.shiftLog'))];
                case 2:
                    pool = _b.sent();
                    return [4 /*yield*/, pool
                            .request()
                            .input('userName', newUserName)
                            .input('recordCreate_userName', user.userName)
                            .input('recordCreate_dateTime', currentDate)
                            .input('recordUpdate_userName', user.userName)
                            .input('recordUpdate_dateTime', currentDate).query(/* sql */ "\n        insert into ShiftLog.Users (\n          userName,\n          recordCreate_userName, recordCreate_dateTime,\n          recordUpdate_userName, recordUpdate_dateTime\n        ) values (@userName,\n          @recordCreate_userName, @recordCreate_dateTime,\n          @recordUpdate_userName, @recordUpdate_dateTime\n        )\n        ")];
                case 3:
                    _b.sent();
                    return [2 /*return*/, true];
                case 4:
                    _a = _b.sent();
                    return [2 /*return*/, false];
                case 5: return [2 /*return*/];
            }
        });
    });
}
function restoreDeletedUser(newUserName, user) {
    return __awaiter(this, void 0, void 0, function () {
        var currentDate, pool, result;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    currentDate = new Date();
                    return [4 /*yield*/, mssql_multi_pool_1.default.connect((0, config_helpers_js_1.getConfigProperty)('connectors.shiftLog'))];
                case 1:
                    pool = _a.sent();
                    return [4 /*yield*/, pool
                            .request()
                            .input('userName', newUserName)
                            .input('recordUpdate_userName', user.userName)
                            .input('recordUpdate_dateTime', currentDate).query(/* sql */ "\n      update ShiftLog.Users\n        set recordUpdate_userName = @recordUpdate_userName,\n        recordUpdate_dateTime = @recordUpdate_dateTime,\n        recordDelete_userName = null,\n        recordDelete_timeMillis = null\n      where userName = @userName\n      ")];
                case 2:
                    result = _a.sent();
                    return [2 /*return*/, result.rowsAffected.length > 0];
            }
        });
    });
}
function addUser(newUserName, user) {
    return __awaiter(this, void 0, void 0, function () {
        var pool, recordDeleteResult, success;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, mssql_multi_pool_1.default.connect((0, config_helpers_js_1.getConfigProperty)('connectors.shiftLog'))
                    // Check if an user with the same name already exists
                ];
                case 1:
                    pool = _a.sent();
                    return [4 /*yield*/, pool
                            .request()
                            .input('userName', newUserName).query(/* sql */ "\n      select recordDelete_dateTime from ShiftLog.Users where userName = @userName\n    ")];
                case 2:
                    recordDeleteResult = (_a.sent());
                    success = false;
                    if (!(recordDeleteResult.recordset.length === 0)) return [3 /*break*/, 4];
                    return [4 /*yield*/, insertNewUser(newUserName, user)];
                case 3:
                    success = _a.sent();
                    return [3 /*break*/, 6];
                case 4:
                    if (!(recordDeleteResult.recordset[0].recordDelete_dateTime !== null)) return [3 /*break*/, 6];
                    return [4 /*yield*/, restoreDeletedUser(newUserName, user)];
                case 5:
                    success = _a.sent();
                    _a.label = 6;
                case 6: return [2 /*return*/, success];
            }
        });
    });
}
