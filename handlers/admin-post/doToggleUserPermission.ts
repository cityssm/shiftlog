...
export default async function handler(
  ...
  try {
    ...
    const updateForm = {
      ...
      shifts_canView:
        permissionField === 'shifts_canView'
          ? !currentUser.shifts_canView
          : currentUser.shifts_canView,

      shifts_canUpdate:
        permissionField === 'shifts_canUpdate'
          ? !currentUser.shifts_canUpdate
          : currentUser.shifts_canUpdate,

      shifts_canManage:
        permissionField === 'shifts_canManage'
          ? !currentUser.shifts_canManage
          : currentUser.shifts_canManage,

      workOrders_canView:
        permissionField === 'workOrders_canView'
          ? !currentUser.workOrders_canView
          : currentUser.workOrders_canView,

      workOrders_canUpdate:
        permissionField === 'workOrders_canUpdate'
          ? !currentUser.workOrders_canUpdate
          : currentUser.workOrders_canUpdate,

      workOrders_canManage:
        permissionField === 'workOrders_canManage'
          ? !currentUser.workOrders_canManage
          : currentUser.workOrders_canManage,

      timesheets_canView:
        permissionField === 'timesheets_canView'
          ? !currentUser.timesheets_canView
          : currentUser.timesheets_canView,

      timesheets_canUpdate:
        permissionField === 'timesheets_canUpdate'
          ? !currentUser.timesheets_canUpdate
          : currentUser.timesheets_canUpdate,

      timesheets_canManage:
        permissionField === 'timesheets_canManage'
    ...
    const success = await updateUser(updateForm, request.session.user as User)
