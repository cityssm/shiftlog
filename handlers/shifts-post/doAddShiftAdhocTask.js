...
export default async function handler(request, response) {
    ...
    const alreadyExists = await isAdhocTaskOnShift(request.body.shiftId, request.body.adhocTaskId);
    if (alreadyExists) {
        response.json({
            success: false,
    ...
    const success = await addShiftAdhocTask(request.body.shiftId, request.body.adhocTaskId, request.body.shiftAdhocTaskNote);
    if (success) {
