 ...
 */
async function generateWorkOrderDigestEmail(reportParameters, apiKey, applicationUrl) {
    ...
    // Calculate counts
    const overdueWorkOrdersCount = digestData.workOrders.filter((wo) => wo.isOverdue).length;
    const newWorkOrdersCount = digestData.workOrders.filter((wo) => wo.isNew).length;
    const overdueMilestonesCount = digestData.milestones.filter((m) => m.isOverdue).length;
        ...
        <strong>${digestData.workOrders.length}</strong> open ${workOrdersSectionName.toLowerCase()}`;
            if (overdueWorkOrdersCount > 0 || newWorkOrdersCount > 0) {
                ...
                if (overdueWorkOrdersCount > 0) {
                    parts.push(`<span style="color: #ff3860; font-weight: bold;">${overdueWorkOrdersCount} overdue</span>`);
                }
                if (newWorkOrdersCount > 0) {
                    parts.push(`<span style="color: #3298dc; font-weight: bold;">${newWorkOrdersCount} new</span>`);
                }
                html += ` (${parts.join(', ')})`;
            }
            html += '</li>';
        ...
        <strong>${digestData.milestones.length}</strong> open milestones`;
            if (overdueMilestonesCount > 0 || newMilestonesCount > 0) {
                ...
                if (overdueMilestonesCount > 0) {
                    parts.push(`<span style="color: #ff3860; font-weight: bold;">${overdueMilestonesCount} overdue</span>`);
                }
                if (newMilestonesCount > 0) {
                    parts.push(`<span style="color: #3298dc; font-weight: bold;">${newMilestonesCount} new</span>`);
                }
                html += ` (${parts.join(', ')})`;
            }
            html += '</li>';
    ...
    try {
        ...
        switch (report.reportType) {
            case 'workOrderDigest': {
                const emailData = await generateWorkOrderDigestEmail(report.reportParametersJson ?? {}, apiKey, applicationUrl);
                html = emailData.html;
